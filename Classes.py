...
class MultipleGameSets:
    ...
    def __init__(self, ids, prob_head, n_games_in_a_set):
        ...
        self._mean_of_rewards = []  # create an empty list where the average of the rewards will be stored
    ...
    def simulation(self):
        for i in range(len(self._ids)):
            ...
            self._mean_of_rewards.append(outcomes.get_ave_reward())

        # summary statistics of total rewards
        self._sumStat_totalRewards = Stat.SummaryStat("Mean Rewards", self._totalRewards)
        # summary statistics of the mean of the means of rewards
        self._sumStat_mean_of_rewards = Stat.SummaryStat("Mean Rewards", self._mean_of_rewards)


    def get_mean_of_rewards(self):
        return self._mean_of_rewards

    # get the mean of total rewards
    def get_double_mean_of_rewards(self):
        return self._sumStat_mean_of_rewards.get_mean()

    # get prediction interval for total reward
    def get_PI_mean_of_rewards(self, alpha):
        return self._sumStat_mean_of_rewards.get_PI(alpha)
